...
import useCalculadoraStore, { TAXA_EMPLACAMENTO, VALOR_PLACA, VALOR_HONORARIOS, VALOR_TAXA_ALIENACAO } from '../store/calculadoraStore';
...
const Calculadora = () => {
  ...
  return (
    <div className="calculadora-carros max-w-3xl mx-auto p-8 bg-white border border-gray-200 rounded-xl my-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-7">
        <div className="space-y-5">
          <div className="transition-all duration-200 hover:scale-[1.01]">
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-14 pr-4 py-3 bg-white border border-gray-300 rounded-lg text-lg font-medium text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="0,00"
                required
              />
          ...
          <div className="transition-all duration-200 hover:scale-[1.01]">
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className={`flex items-center p-4 border rounded-lg cursor-pointer transition-all duration-200 ${tipoVenda === 'direta' ? 'bg-blue-50 border-blue-400' : 'hover:bg-gray-50'}`}>
                <input
                  type="radio"
                  value="direta"
              ...
              <label className={`flex items-center p-4 border rounded-lg cursor-pointer transition-all duration-200 ${tipoVenda === 'estoque' ? 'bg-blue-50 border-blue-400' : 'hover:bg-gray-50'}`}>
                <input
                  type="radio"
                  value="estoque"
          ...
          <div className={`p-4 border rounded-lg hover:bg-gray-50 cursor-pointer transition-all duration-200 ${taxaAlienacao ? 'bg-blue-50 border-blue-400' : ''}`}>
            <label className="flex items-center cursor-pointer">
              ...
              />
              <span className="ml-3 text-gray-700 font-medium">Incluir Taxa de Alienação <span className="text-blue-700">(R$ {VALOR_TAXA_ALIENACAO.toLocaleString('pt-BR', { minimumFractionDigits: 2 })})</span></span>
            </label>
          </div>
        </div>
        ...
        <div className="border rounded-lg p-5 bg-gray-50">
          ...
          <ul className="text-sm text-gray-700 space-y-2 pl-7">
            <li className="flex items-center">
              ...
              Taxa de primeiro emplacamento: <span className="font-medium ml-1">R$ {TAXA_EMPLACAMENTO.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </li>
            <li className="flex items-center">
              <div className="w-2 h-2 bg-blue-600 rounded-full mr-2"></div>
              Placa: <span className="font-medium ml-1">R$ {VALOR_PLACA.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </li>
            <li className="flex items-center">
              <div className="w-2 h-2 bg-blue-600 rounded-full mr-2"></div>
              Honorários: <span className="font-medium ml-1">R$ {VALOR_HONORARIOS.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </li>
          </ul>
        </div>
        ...
        <div className="border rounded-lg p-5 bg-blue-50 space-y-4">
          ...
          <div className="border-t border-gray-200 pt-5">
            <div className="flex justify-between items-center bg-blue-700 p-4 rounded-lg text-white">
        ...
        <button
          type="submit"
          className="w-full bg-blue-700 text-white py-4 px-6 rounded-lg hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-300 flex items-center justify-center font-semibold text-lg"
        >
